...
const toTreeData = (dataList = [], config = {}) => {
  ...
  for (let i = 0; i < dataList.length; i++) {
    ...
    if (row.id == row.pId || _.isUndefined(row.pId)) {
      items.push(item);
    }
    idMap[row.id] = item;
  }
...
const _parseItemValue = (value, dataType) => {
  ...
  const getDateStamp = (date, stampType) => {
    try {
      switch (stampType) {
        case dateStart:
          return date.set({ 'hour': 0, 'minute': 0, 'second': 0, 'millisecond': 0 }).valueOf();
        case dateEnd:
          return date.set({ 'hour': 23, 'minute': 59, 'second': 59, 'millisecond': 999 }).valueOf();
        default:
          return date.valueOf();
      }
    } catch (e) {
...
const buildFilterParams = (...args) => {
  let _params = _.assign({}, ...args);
  let { filterGroup = { rules: [] }, searchRule = {} } = {};
  let { rules } = filterGroup;
      ...
      if (fieldConfig) {
        ...
        if (field) {
          if (rangeIndex != null) {
            ...
            fieldObj[realKey] = arrValues;
          } else {
            fieldObj[realKey] = { value: val };
          }
        }
      } else {
        fieldObj[key] = { value: value };
      }
    }
  };
